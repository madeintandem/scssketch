...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/export-to-sass.js");
/******/ })
/************************************************************************/
/******/ ({
...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  var formElements = __webpack_require__(/*! ./internal/formElements */ "./src/internal/formElements.js");
  ...
  var formElementMap = formElements.parse(sharedStyles, sharedTextStyles, layerStyleMap);
  var formElementStyleSheet = formElements.writeSass(formElementMap);
...
module.exports = {
  ...
  addColors: function addColors(colorStyles) {
    return _.reduce(colorStyles, function (colors, style) {
      var tagName = common.getTag(String(style.name())); // TODO: @Drew just ran into a scenario that this is null
  ...
  writeColors: function writeColors(colors) {
    ...
    var styles = _.reduce(colors, function (styles, color) {
      return styles + "$".concat(color.name, ": ").concat(color.value, ";\n");
    }, "// COLORS\n");
...
module.exports = {
  parse: function parse(sharedStyles, sharedTextStyles, layerStyles) {
    if (!symbolsPage) {
    ...
    var buttonSymbols = _.filter(symbolsPage.layers(), function (layer) {
  ...
  writeSass: function writeSass(styles) {
    ...
    _.forEach(styles, function (style) {
      ...
      _.forEach(style.attributes.borderThickness, function (val) {
        borderWidth += val + "px ";
      });
      ...
      _.forEach(style.attributes.borderRadius, function (val) {
        borderRadius += val + "px ";
      });
      ...
      _.forEach(style.attributes.padding, function (val, i) {
...
function getElementAttributes(elements, sharedStyles, sharedTextStyles, layerStyles) {
  ...
  _.forEach(elements, function (element) {
    ...
    var shape = _.find(element.layers(), function (layer) {
    ...
    if (shape) {
      ...
      if (shapeAttributes.background) {
        elementAttributes.background = shapeAttributes.background;
      }

      if (shapeAttributes.borderColor) {
        elementAttributes.borderColor = shapeAttributes.borderColor;
      }

      if (shapeAttributes.borderRadius) {
        elementAttributes.borderRadius = shapeAttributes.borderRadius;
      }

      if (shapeAttributes.borderThickness) {
        elementAttributes.borderThickness = shapeAttributes.borderThickness;
      }
    } // Check for text layer


    var textLayer = _.find(element.layers(), function (layer) {
      return String(layer.name()).toLowerCase() === "label" || String(layer.name()).toLowerCase() === "value";
    });
    ...
    if (!textLayer) {
      ...
      var textStyle = _.find(sharedTextStyles.objects(), function (textStyle) {
        return String(textStyle.objectID()) === String(textLayer.sharedStyleID());
      });

      if (textStyle) {
        var tag = common.getTag(textStyle.name());
        elementAttributes.textStyle = tag.cssSelector + "-text-style";
      } // get color
      ...
      elementAttributes.textColor = fontColor; // get alignment
      ...
      elementAttributes.textAlignment = fontAlignment; // get padding!!!

      var getText = _.find(element.layers(), function (layer) {
        return String(layer.name()).toLowerCase() === "label" || String(layer.name()).toLowerCase() === "value";
      });
      ...
      if (getText) {
        ...
        rightPadding = parseInt(getText.frame().x()), topPadding = parseInt(getText.frame().y()), symbolTopOffset = 0, symbolBottomOffset = 0; // check for symbol
        ...
        elementAttributes.padding = [topPadding + symbolTopOffset, leftPadding, parseInt(element.frame().height()) - (parseInt(getText.frame().height()) + topPadding + symbolTopOffset), rightPadding];
      }
    }
...
function findLayerStyleByColor(color, styles) {
  var result = _.find(styles, function (style) {
...
function findSymbolById(theid) {
  return _.find(symbolsPage.layers(), function (layer) {
...
function getShapeOverrides(shapeSymbol, sharedStyles) {
  ...
  if (shapeSymbol.overrideValues && shapeSymbol.overrideValues().length > 0) {
    _.forEach(shapeSymbol.overrideValues(), function (override) {
  ...
  var symbolOverrides = _.filter(overrides, function (override) {
    return override.overrideType.startsWith("symbol") && override.overrideValue.length;
  });

  var layerStyleOverrides = _.filter(overrides, function (override) {
    return override.overrideType.startsWith("layerStyle") && override.overrideValue.length;
  });

  _.forEach(symbolOverrides, function (override) {
  ...
  _.forEach(layerStyleOverrides, function (override) {
    var style = _.find(sharedStyles.objects(), function (style) {
      return String(style.objectID()) === override.overrideValue;
    });

    var layerToOverride = _.find(symbolMaster.layers(), function (layer) {
    ...
    if (layerToOverride && style) {
      ...
      if (String(colorValue.name).toLowerCase() === "none" || String(colorValue.name).toLowerCase() === "[x] none") {
...
function getBorderThickness(border) {
  ...
  return [Math.abs(outer.top - inner.top), Math.abs(outer.bottom - inner.bottom), Math.abs(outer.left - inner.left), Math.abs(outer.right - inner.right)];
}
...
function parseShape(layer, sharedStyles, layerStyles) {
  var radius,
      ...
      borderThickness,
      borderColor = "transparent",
      shapeSymbolMaster = findSymbolById(layer.symbolID()); // We need to get the fill symbol

  var fillLayer = _.find(shapeSymbolMaster.layers(), function (layer) {
  ...
  if (fillLayer) {
    ...
    _.forEach(fillLayer.points(), function (point) {
  ...
  var border = _.find(shapeSymbolMaster.layers(), function (layer) {
...
module.exports = {
  ...
  writeGradients: function writeGradients(gradients) {
    var styles = "";
...
var colors = __webpack_require__(/*! ./colors */ "./src/internal/colors.js");

var shadows = __webpack_require__(/*! ./shadows */ "./src/internal/shadows.js");

var gradients = __webpack_require__(/*! ./gradients */ "./src/internal/gradients.js");
...
module.exports = {
  parse: function parse(sharedStyles) {
    ...
    var shadowStyles = _.filter(sortedStyles, function (style) {
      return shadows.isShadow(style);
    });

    var otherStyles = _.differenceWith(sortedStyles, shadowStyles, _.isEqual);

    var gradientStyles = _.filter(otherStyles, function (style) {
  ...
  writeSass: function writeSass(layerStyleMap) {
    return "".concat(colors.writeColors(layerStyleMap.colors)).concat(gradients.writeGradients(layerStyleMap.gradients)).concat(shadows.writeShadows(layerStyleMap.shadows));
  }
};
...
module.exports = {
  parse: function parse(sharedTextStyles) {
    ...
    _.forEach(getUniqueStyles(sortedStyles), function (thisStyle) {
  ...
  writeSass: function writeSass(layerTextStyleMap, fonts) {
    ...
    if (layerTextStyleMap.desktop.styles && layerTextStyleMap.desktop.styles.length || layerTextStyleMap.mobile.styles && layerTextStyleMap.mobile.styles.length || layerTextStyleMap.assorted.styles && layerTextStyleMap.assorted.styles.length) {
      ...
      _.forEach(fonts, function (font) {
        if (outputFontWeight) {
          theFontWithWeight = getFontAndWeight(font.fontObject.font); // TODO check if theFontWithWeight.fontFamily is the same as another font family?
  ...
  determineFontType: function determineFontType(foundFonts) {
    ...
    if (foundFonts.length === 1) {
      textFont = {
        "name": "text-font",
        "fontObject": foundFonts[0]
      };
    } else if (foundFonts.length === 2) {
      if (_.find(foundFonts, function (font) {
        return font.isParagraph;
      })) {
        textFont = {
          "name": "text-font",
          "fontObject": _.find(foundFonts, function (font) {
            return font.isParagraph;
          })
        };
      } else {
        textFont = {
          "name": "text-font",
          "fontObject": _.min(foundFonts, function (font) {
            return font.smallestSize;
          })
        };
      }
      ...
      displayFont = {
        "name": "display-font",
        "fontObject": displayFont[0]
      };
    } else {
      // there are more than two fonts
      var paragraphFont = _.find(foundFonts, function (obj) {
        return obj.isParagraph;
      });

      if (paragraphFont) {
        textFont = {
          "name": "text-font",
          "fontObject": paragraphFont
        };
        subArray = _.pull(subArray, paragraphFont);
      }

      _.forEach(subArray, function (font) {
        if (!displayFont && font === _.max(foundFonts, function (font) {
          return font.count;
        }) || font === displayFont) {
          displayFont = {
            "name": "display-font",
          ...
          auxiliaryFont.push({
            "name": "auxiliary-font-" + (auxiliaryFont.length + 1),
            "fontObject": font
          });
        }
      });
...
function getUniqueStyles(styles) {
  return _.uniqBy(styles, function (style) {
...
function popPToTop(styles) {
  styles = _.sortBy(styles, function (style) {
    return common.getTag(String(style.name)).selector === 'p' ? 0 : 1;
  });

  var hasParagraph = _.find(styles, function (style) {
    return common.getTag(String(style.name)).selector === 'p';
  });
...
function getFontAndWeight(fontName) {
  ...
  if (fontName.indexOf("-")) {
    ...
    fontWeightWord = String(fontWeightWord[1]).replace(/[\s]/g, '').toLowerCase(); // find indexOf("italic")
...
function outputSetupVars(style, baseSize, fonts) {
  ...
  var fontType = _.find(fonts, function (font) {
    return font.fontObject.font.replace(/['"]+/g, '') === style.font;
  });
...
module.exports = {
  isShadow: function isShadow(style) {
    return style.value().shadows().length || style.value().innerShadows().length;
  },
  addShadows: function addShadows(shadowStyles) {
    return _.reduce(shadowStyles, function (shaddows, style) {
      var thisName = String(style.name());
      var tag = common.getTag(thisName);
  ...
  writeShadows: function writeShadows(shadows) {
    var styles = "";
...
//# sourceMappingURL=export-to-sass.js.map